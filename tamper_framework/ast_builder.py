...
def reconstruct_from_ast(node: ASTNode) -> str:
    ...
    FIXED: Uses stable sort with index tie-breaker to handle
    edge cases where multiple items have the same position
    """
    ...
    # Add all tokens with their positions and insertion order
    for idx, token in enumerate(node.tokens):
        items.append((token.position, idx, token.value))
    
    # Add all children with their first token's position
    # Use negative index to ensure children come after tokens at same position
    for idx, child in enumerate(node.children):
        if child.tokens:
            first_pos = child.tokens[0].position
            items.append((first_pos, 1000 + idx, reconstruct_from_ast(child)))
        elif child.children:
            # Child has no tokens but has children - recurse
            items.append((0, 1000 + idx, reconstruct_from_ast(child)))
    
    # Sort by position, then by insertion order (stable sort)
